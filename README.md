...
```mermaid
  graph LR;
      ...
      RPYJU--> RSTUDIORPYJU[rstudio-r-python-julia]:::ide;
...
- use an init script : https://github.com/InseeFrLab/images-datascience/blob/main/base/scripts/onyxia-init.sh
...
- put this path in env variable PATH_TO_CA_BUNDLE then onyxia-init.sh script will configure git and pip to user this certificates.
...
- kubernetes injection: you rely on some kubernetes global certificate injection then use kyverno/gatekeeper to enforce PATH_TO_CA_BUNDLE to the injected path.
  - soon PATH_TO_CA_BUNDLE will be an injected field of the onyxia region.

- rebuild of images just to add certificates: you can also rebuild the images in your entreprise just to add ca certificates with this same recipe relying on a PATH_TO_CA_BUNDLE.
